...
# Benchmarks
[CSV with the results](doc/benchmark.csv). You can see the workflow file [here](.github/workflows/cmake.yml). It compiles the `benchmark` target using GCC-12 (actual version is saved to the [doc/benchmark.csv](doc/benchmark.csv)). The target is configured first with `Release` build type which forces `-O2`. Binary size is obtained using the `size` command, and total readingh is plotted on the picture below. Compilation time is taken using the `time -f%e ...` command (installed in additional step. Built-in time command takes no arguments). Eventually the `time` command is applied on the binary itself (5000 keypresses are simulated). Then everything repeats for the `MinSizeRel` nuild type. 
